...
class Runner:
    ...
    def run(self):
        ...
        if save:
            if os.path.exists("demonstrations/{}.pkl".format(self.demonstrations_name)):
                y = input("do you want to load previous dataset?")
                if y == 'y':
                    a_file = open("demonstrations/{}.pkl".format(self.demonstrations_name), "rb")
                    self.trajectories = pickle.load(a_file)
        ...
        while self.ep <= self.total_episode:
            ...
            # TODO: save trajectories to file
            if save:
                if traj is not None:
                    self.trajectories.append(traj)
                    a_file = open("demonstrations/{}.pkl".format(self.demonstrations_name), "wb")
                    pickle.dump(self.trajectories, a_file)
                    a_file.close()
            ...
            while True:
                ...
                if not self.recurrent:
                    action, logprob, probs, dist = self.agent(torch.from_numpy(np.asarray(state)).to(device).float())
                else:
                ...
                if self.random_actions is not None and self.total_step < self.random_actions:
                    if self.agent.action_type == 'discrete':
                        ...
                        action = [np.random.uniform(-1, 1, self.agent.action_size)]
                ...
                if save:
                    ...
                    traj['next_states'].append(state_n)
