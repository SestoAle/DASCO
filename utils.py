...
def save_statistics(name_run, p_losses, c_losses, rewards, d4rl_rewards, folder=""):
    stats = dict(name_run=name_run, p_losses=p_losses, c_losses=c_losses, rewards=rewards, d4rl_rewards=d4rl_rewards)

    a_file = open("{}/{}.pkl".format(folder, name_run), "wb")
...
class DTDemDataset:
    def __init__(self, trajectories, context_len, rtg_scale, state_norm=True, gamma=1.):
        ...
        for traj in self.trajectories:
            ...
            traj['returns_to_go'] = np.reshape(traj['returns_to_go'], (-1,))
        ...
        if state_norm:
            ...
            for traj in self.trajectories:
                for i in range(len(traj['states'])):
                    traj['states'][i] = (traj['states'][i] - self.state_mean) / self.state_std
    ...
    def get_item(self, idx):
        ...
        if traj_len >= self.context_len:
            ...
            states = torch.from_numpy(np.asarray(traj['states']))
